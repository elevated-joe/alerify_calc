...
export function priceServer(v: ServerConfig, addons: Addons, keyed: Map<string, Instance>): Priced {
  ...
  // Backups. Backup data size = full base copy + daily increments:
  // EBS × (1 + daily change rate × retention days).
  if (v.localBackup) {
    const days = v.backupRetentionDays > 0 ? v.backupRetentionDays : 30;
    const backupGB = v.ebs * (1 + addons.backupDailyChangeRate * days);
    ...
    add("Local backup", `${Math.round(backupGB)} GB · ${v.ebs} GB full + ${changePct}%/day × ${days} days`,
